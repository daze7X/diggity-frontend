...
export default function UserProfile() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (password && password !== passwordConfirmation) {
            setStatus('error');
            setMessage('Konfirmasi sandi baru tidak cocok.');
            return;
        }
        ...
        try {
            ...
            setStatus('error');
            setMessage(err.message || 'Gagal memperbarui profil. Silakan coba kembali.');
        }
    };
    ...
    return (
        <div className="space-y-6 text-left animate-fade-in">
            <div>
                <h2 className="text-xl md:text-2xl font-extrabold text-text-main tracking-tight">Pengaturan Profil</h2>
                <p className="text-xs md:text-sm text-text-muted">Kelola detail identitas akun Anda dan ubah kata sandi secara aman.</p>
            </div>
            ...
            <SpotlightCard className="p-8 border border-glass-border">
                <form onSubmit={handleSubmit} className="space-y-5 text-xs md:text-sm">
                    ...
                    <div className="border-t border-glass-border/40 my-6 pt-4 space-y-4">
                        <div>
                            <h4 className="text-sm font-bold text-text-main">Ubah Kata Sandi (Opsional)</h4>
                            <p className="text-[10px] text-text-muted">Biarkan kosong jika Anda tidak ingin mengubah sandi saat ini.</p>
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {/* Password Input */}
                            <div className="space-y-1.5">
                                <label className="font-bold text-text-gray block text-xs">Kata Sandi Baru</label>
                                <div className="relative">
                                    <input
                                        type="password"
                                        value={password}
                            ...
                            <div className="space-y-1.5">
                                <label className="font-bold text-text-gray block text-xs">Konfirmasi Sandi Baru</label>
                                <div className="relative">
                                    <input
                                        type="password"
