...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    try {
        ...
        const metaDesc = blog.seo_meta?.meta_description || blog.meta_description || 'Baca selengkapnya berita resmi Diggity.';
        const canonical = blog.seo_meta?.canonical_url || '';

        return {
            title: `${metaTitle} | Diggity News`,
        ...
        return {
            title: 'Berita & Pengumuman | Diggity News',
        };
    }
}
...
export default async function NewsDetailPage({ params }: Props) {
    ...
    try {
        news = await api.getInsightBySlug(slug);
        if (news) {
            const currentNews = news; // Non-nullable constant for closure
            const allBlogs = await api.getInsights();
            // Filter: only related news
            relatedNews = allBlogs
                .filter(
                    (b: any) => 
                        b.id !== currentNews.id && 
                        (b.category?.slug === 'berita-pengumuman' || b.category?.name === 'Berita & Pengumuman')
                )
                .slice(0, 3);
        }
    } catch (error) {
        console.error('Error fetching news details / related posts:', error);
    }
    ...
    return (
        <div className="relative pt-36 pb-20 md:pt-40 md:pb-28">
            {news.seo_meta?.json_ld_schema && (
                <script
                    type="application/ld+json"
                    dangerouslySetInnerHTML={{ 
                        __html: typeof news.seo_meta.json_ld_schema === 'string'
                            ? news.seo_meta.json_ld_schema 
                            : JSON.stringify(news.seo_meta.json_ld_schema) 
                    }}
                />
            )}
