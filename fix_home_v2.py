...
# Find the start of `<div className="relative">` up to `{/* 02.`
# NOTE: In the original, the hero section ends with `</section>`.
pattern = re.compile(r'<div className="relative">\s*\{/\* 01\. Hero Section \*/\}[\s\S]*?</section>\s*(?=\{/\* 02\.)')
content = pattern.sub(new_hero + "\n                ", content)

# Also close the div at the very bottom
content = content.replace("            {/* 15. Final CTA & Contact Section */}", "            </div>\n\n            {/* 15. Final CTA & Contact Section */}")

with open(path, "w", encoding="utf-8") as f:
    f.write(content)

print("Replaced Home page hero.")
