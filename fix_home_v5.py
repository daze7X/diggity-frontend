...
# We need to remove the `</div>` and `</section>` that closed the hero section, because now they are just part of the big -mt-24 div.
...
new_end_hero_str = """                </div>

            {/* 03. What We Do Section */}"""

content = content.replace(end_hero_str, new_end_hero_str)

# Part 3: Close the overarching `min-h-screen` and `max-w-7xl` divs at the very end
