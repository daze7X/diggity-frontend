...
# Replace all imports up to export const revalidate
content = re.sub(r"import React from 'react';.*?(?=export const revalidate = 60;)", imports_new + '\n\n', content, flags=re.DOTALL)
            ...
            <div className="bg-brand-blue dark:bg-brand-bg dark:border-b dark:border-glass-border relative pt-32 pb-32 px-6 overflow-hidden">
                ...
                <div className="max-w-7xl mx-auto relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-10">
                    ...
                    <div className="hidden md:flex shrink-0">
                        <div className="w-64 h-64 bg-white/5 border border-white/10 rounded-full flex items-center justify-center relative shadow-2xl backdrop-blur-sm">
                            <div className="absolute inset-0 bg-gradient-to-tr from-transparent to-white/20 rounded-full animate-spin-slow" style={{ animationDuration: '15s' }} />
                            <Cpu className="w-24 h-24 text-white/50" />
                        </div>
                    </div>
                </div>
            </div>
...
content = content[:return_start] + return_new

with open(filepath, 'w', encoding='utf-8') as f:
    f.write(content)
print("Product detail page transformed to enterprise style.")
